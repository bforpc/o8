...
2. Create the private application state directory before opening the installer. `storage/system/` is deliberately inside the project: it contains only the installation identity, encrypted source/AI credentials, setup lock and PHP sessions — never tenant document originals. Tenant document storage is configured later by the operator outside the project and web root. For Apache on Debian this is typically:

   ```sh
   sudo install -d -o www-data -g www-data -m 0770 /absolute/path/to/o8/storage/system
   ```
