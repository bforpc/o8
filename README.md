...
> The feature descriptions below explain the intended finished product. M2.3 adds operator-managed storage and a first database-backed four-column workspace: test-file uploads, original previews/downloads, metadata, basic folders/tags and trash. The complete M1 demo remains separate. Full search/settings parity, bookkeeping, imports and recovery jobs are still pending. Use trusted test files only; this is not production-ready. See [storage setup and limits](docs/STORAGE_AND_DOCUMENTS.md).
>
> **Entwicklungsstand — M1 abgenommen, weiterhin Entwicklungsvorschau, keine Produktivversion.**
> Die folgenden Funktionen beschreiben das Zielprodukt. M2.3 ergänzt Storage-Zuweisung durch den Betreiber und einen ersten datenbankgestützten Vier-Spalten-Arbeitsplatz: Testdatei-Upload, Originalvorschau/-download, Metadaten, einfache Ordner/Tags und Papierkorb. Die vollständige M1-Demo bleibt separat. Vollständige Suche/Einstellungen, Buchungen, Importe und Wiederaufnahme-Jobs folgen. Nur vertrauenswürdige Testdateien verwenden; keine Produktivfreigabe. Siehe [Storage-Einrichtung und Grenzen](docs/STORAGE_AND_DOCUMENTS.md).
...
- **Independent tenants and clear permissions:** one user login, followed by a choice of authorized tenants (automatic entry when there is only one). Memberships, roles, documents, folders, tags, bookkeeping, sources and settings remain tenant-scoped. Tenant switching is available after login. Operator-level administration stays separate from document access. See the [account model](docs/ACCOUNTS.md).
...
| M2 — In progress | Installer, accounts, storage and first real test-document workspace available; full M1 feature integration pending |
...
The M2.3 backend requires PDO MySQL, mbstring, fileinfo, POSIX, sessions and MariaDB 10.6+ or MySQL 8.0+ (tested with MariaDB 11.8). A web installer, `config.php.dist`, operator-managed storage assignment and a first real test-document workspace are available. Create a dedicated empty database, then run `php bin/setup.php token` locally and open the root page. The local setup code protects installation and the first `admin` / `owndms8` login; changing that password is mandatory. See [installation details](docs/INSTALLATION.md) and [storage limits](docs/STORAGE_AND_DOCUMENTS.md). Never publish `config.php` or `storage/system`. Automatic imports, bookkeeping parity and trash jobs remain unavailable.
...
- **Unabhängige Mandanten und klare Rechte:** gemeinsamer Benutzer-Login, danach Auswahl der berechtigten Mandanten (bei genau einem direkter Einstieg). Zuordnungen, Rollen, Dokumente, Ordner, Tags, Buchungen, Quellen und Einstellungen bleiben mandantenbezogen. Ein Wechsel ist auch nach dem Login möglich. Die Betreiberverwaltung bleibt vom Dokumentzugriff getrennt. Siehe [Kontomodell](docs/ACCOUNTS.md).
...
| M2 — In Arbeit | Installer, Konten, Storage und erster Arbeitsplatz für echte Testdokumente vorhanden; vollständige M1-Anbindung folgt |
...
Das M2.3-Backend benötigt PDO MySQL, mbstring, fileinfo, POSIX, Sessions und MariaDB 10.6+ (getestet mit MariaDB 11.8). Webinstaller, `config.php.dist`, geprüfte Storage-Zuweisung und ein erster echter Testdokument-Arbeitsplatz sind vorhanden. Eigene leere Datenbank bereitstellen, lokal `php bin/setup.php token` ausführen und die Startseite öffnen. Der Einrichtungscode schützt Installation und erste Anmeldung als `admin` / `owndms8`; anschließend ist ein Passwortwechsel zwingend. Siehe [Installationsanleitung](docs/INSTALLATION.md) und [Storage-Grenzen](docs/STORAGE_AND_DOCUMENTS.md). `config.php` und `storage/system` niemals veröffentlichen. Automatische Importe, vollständige Buchungsparität und Papierkorb-Jobs folgen später.
