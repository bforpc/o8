...
$root=dirname(__DIR__); $message=''; $error=''; $installed=false; $actor=null; $fatal=false; $tenants=[]; $users=[];
...
try {
    ...
    $runtime=new Runtime($root.'/storage/system'); $identity=$runtime->identity();
    ...
    if (!$actor) unset($_SESSION['actor'],$_SESSION['tenant_delete_review'],$_SESSION['invite_code']);
    ...
    if ($_SERVER['REQUEST_METHOD']==='POST') {
        try {
            ...
            $_SESSION['csrf']=bin2hex(random_bytes(32));
            $destination=in_array(field('section'),['tenants','users','account','storage','accounting','documents'],true)?'?section='.field('section'):'';
    ...
    if ($actor && !$actor->row['must_change_password'] && !($actor->row['bootstrap_pending']??false)) {
        ...
        $allowed=$actor->kind==='operator'?['tenants','storage','account']:($actor->kind==='account'?['choose','account']:($actor->row['role']==='admin'?['documents','users','accounting','account','choose']:['documents','account','choose']));
